#'
linear.log_likelihood <- function(coef, x, y, noise_var = 1) {
  n <- length(y)
...
#'
linear.log_likelihood.gradient <-
...
linear.mle.BFGS <- function(design, outcome) {
  op <- stats::optim(
    ...
    fn = function(par)
      linear.log_likelihood(par, design, outcome),
    gr = function(par)
      linear.log_likelihood.gradient(par, design, outcome),
